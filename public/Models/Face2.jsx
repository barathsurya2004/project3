...
export function Face2(props) {
  ...
  const ori = useRef({ alpha: 0, beta: 0, gamma: 0 });
  const { hovered, setHovered } = useContext(Context);

  useFrame((state) => {
    const { beta, gamma, alpha } = ori.current;
    if (group.current) {
      // Convert degrees to radians if necessary
      // Example: group.current.rotation.x = beta * (Math.PI / 180);
      group.current.rotation.x = beta * (Math.PI / 180);
      group.current.rotation.y = alpha ? alpha * (Math.PI / 180) : 0; // Adjust as needed
      // You can also use gamma if required
    }
